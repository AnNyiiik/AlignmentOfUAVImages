...
class homography_finder_CNN_based(homography_finder.homography_finder):
    ...
    def align(self, img_query_path, img_reference_path):
        ...
        H = homography_finder_feature_based.homography_finder_feature_based().align(img_query_path, img_reference_path)
        ...
        os.system(f'docker cp {query_saved_path} {path_to_data_inside_container}/query.jpg >/dev/null 2>&1')
        os.system(f'docker cp {reference_saved_path} {path_to_data_inside_container}/reference.jpg >/dev/null 2>&1')
        ...
        os.system(f'docker exec {container_id} python code/homography_CNN_real.py --mode=test --batch_size=2 >/dev/null 2>&1')
        os.system(f'docker cp {container_id}:/workspace/log/results/homographies.txt {path_to_save_result_matrix} >/dev/null 2>&1')
